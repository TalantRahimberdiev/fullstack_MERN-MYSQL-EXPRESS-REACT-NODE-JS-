...
export default function DailySales() {
   ...
   const getSales = async () => {
      try {
         const response = await axios.get(`http://localhost:7000/dailySupply/${[day, month, year]}`)
   ...
   return (
      <div>
         <h1 style={{ textAlign: 'center' }}>суточные поставки</h1>
         ...
         {
            dailySales && <>
               <p style={{ textAlign: 'center' }}>поставлено товаров:{quantity}, на общую сумму {totalCost}</p>
               {
