...
export default function UpdateZakazClienta() {
	...
	const getZakaz = async () => {
		try {
			...
			setProduct(tovary && currentZakaz && tovary.filter(item => item.id === currentZakaz[0].id_tovar)[0].product)
		}
	...
	return (
		<div>
			<h1 style={{ textAlign: 'center' }}>Редактировать заказ клиентов</h1>
			<TextField
				value={token}
